...
public class P39_DuplicationInArray {
    ...
    public static int partion(int[] data,int start,int end){
        if(start>=end)
            return end;
        int pivot = data[start];//选择一个基准数，腾出位置（挖坑），寻找比基准数小的元素填充位置（填坑）
    ...
    //方法三：哈希表，不会修改原始数据，时间复杂度o(n),空间复杂度o(n)
    ...
    //若没有重复的数字，则各位置的数字与其序号相对应。否则进行交换排序比较
    ...
    //方法五：类似于二路归并，不修改原始数据，时间复杂度o(nlogn),空间复杂度o(1)
